...
class Model(object):
    ...
    def extract_slice(self, path_data, verbose=False):
        ...
        while True:
            ...
            #Noah's code
            f = open(self.results_dir.replace('results','3D_points').replace('Mesh/','').replace('Model/','') + '3D_points_' + str(point_id) + '.txt','a')
            #End Noah's code
            
            for i in range(num_comp_points):
                pt = comp_points.GetPoint(i)
                dist = ((pt[0] - origin[0])**2 + (pt[1] - origin[1])**2 + (pt[2] - origin[2])**2) ** 0.5
                vtp_points.append(pt)
                #Noah's code
        ...
        #print(self.slice_geometry.GetNumberOfArrays())
        #res = component.GetNumberOfCells()
        #print(component.GetCellData())
        ## Show the slice.
        mapper = vtk.vtkPolyDataMapper()
